...
func (currencyL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCurrency interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"users\".\"id\", \"users\".\"chat_id\", \"users\".\"created_at\", \"users\".\"notification_interval_hours\", \"users\".\"last_notification_time\", \"users\".\"next_notification_time\", \"a\".\"currency_id\""),
		qm.From("\"users\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.ChatID, &one.CreatedAt, &one.NotificationIntervalHours, &one.LastNotificationTime, &one.NextNotificationTime, &localJoinCol)
		if err != nil {
