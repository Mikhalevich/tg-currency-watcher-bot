...
var (
	currencyDBTypes = map[string]string{`ID`: `integer`, `Base`: `text`, `BaseExternalID`: `text`, `Quote`: `text`, `QuoteExternalID`: `text`, `Price`: `numeric`, `UpdatedAt`: `timestamp with time zone`}
	_               = bytes.MinRead
)
...
func testCurrenciesSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(currencyAllColumns, currencyPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, currencyGeneratedColumns)
	}
